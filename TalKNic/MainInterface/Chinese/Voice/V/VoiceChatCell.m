...
- (void)creatUI{
    ...
    // 1.创建头像
    self.avatarView = [[UIImageView alloc]initWithFrame:kCGRM(KMargin, KMargin, kAvatarViewW, kAvatarViewW)];
    self.avatarView.backgroundColor = [UIColor yellowColor];
    self.avatarView.contentMode = UIViewContentModeScaleToFill;
    [self.contentView addSubview:self.avatarView];
    
    // 2.创建名字
    self.nameLabel = [[UILabel alloc]initWithFrame:kCGRM(CGRectGetMaxX(self.avatarView.frame) + KMargin, KMargin, kWidth - kAvatarViewW - KMargin * 4 - KTimeLabelW, KMargin * 2)];
    self.nameLabel.text = @"Name";
    self.nameLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:15.0f];
    [self.contentView addSubview:self.nameLabel];
    
    // 3.剩余时间
    self.leftTimeLabel = [[UILabel alloc]initWithFrame:kCGRM(kWidth - KTimeLabelW, self.nameLabel.frame.origin.y, KTimeLabelW, self.nameLabel.frame.size.height)];
    ...
    // 4.聊天描述
    self.desLabel = [[UILabel alloc]initWithFrame:kCGRM(CGRectGetMaxX(self.avatarView.frame) + KMargin, CGRectGetMaxY(self.nameLabel.frame) + KMargin, kWidth - KMargin * 7 - kRightArrowW - kAvatarViewW, kAvatarViewW - self.nameLabel.frame.size.height - KMargin)];
    self.desLabel.text = @"Audio message!";
    self.desLabel.font = [UIFont fontWithName:@"HelveticaNeue-Medium" size:14.0];
    [self.contentView addSubview:self.desLabel];
    
    // 5.右边箭头
    self.rightIcon = [[UIButton alloc]initWithFrame:kCGRM(kWidth - kRightArrowW - KMargin, CGRectGetMaxY(self.leftTimeLabel.frame), kRightArrowW, kRightArrowW * 1.5)];
...
- (void)setBadge:(NSString *)badge{
    ...
    if ([self.badge isEqualToString:@""] || [self.badge isEqualToString:@"0"] || self.badge == nil) {
        ...
        self.rightIcon.frame = kCGRM(kWidth - kRightArrowW - KMargin, CGRectGetMaxY(self.leftTimeLabel.frame), kRightArrowW, kRightArrowW * 2);
        self.rightIcon.clipsToBounds = NO;
    }else{
        //有聊天通知， 显示小红点
        self.rightIcon.frame = kCGRM(kWidth - kBadgeW - KMargin, CGRectGetMaxY(self.leftTimeLabel.frame) + KMargin, kBadgeW, kBadgeW);
